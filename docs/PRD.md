...
| ~~Is there a second widget family or a Control Center control worth having?~~ **Planned (2026-09-23):** `docs/tallyist-lock-screen-plan.md` — the three Lock Screen families as the watch complication moved to `Shared/`, a − under ADR-0043's rule, and an optional iOS 18 control; 1(a) was resolved (ADR-0047), which was this row's trigger. Seven owner decisions gate its Phase 1. | After 1(a) — a widget whose intent dispatch is unresolved is not a foundation to build on. |
