...
already the calendar's key — synced the same way as below; 369 after the
health pairing's Phase 6 earlier that day: seven app keys in, one out — the fourth row's name, which
...
forms, and the days-only source line; plus the all-adults note re-keyed with
its drinkers share interpolated (`%lld%%`, read from the bundled file), which
...
the widget's five keys into the app catalog, which is the trap to avoid).
Before that, 321 — 256 app — counted
