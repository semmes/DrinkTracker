...
| 8 — release | **Next.** The device pass is in and Phase 7 is closed, so this is the remaining work: What's New (1.4), reviewer notes (the new Background Modes entry included), the claims table re-verified against the watch, the privacy policy read against it, the watch screenshots (the owner's). |
