...
| **Bar detail** | `Trends/PeriodDetailView.swift` | Under the chart, in the same card: the period as a date, a figure, composition rows by kind, or the calendar's four figures for a bucket. The selected bar keeps its accent and the rest dim to 35%; no annotation inside the plot, no delta against the average line, 44pt ✕ to clear (ADR-0028) |
