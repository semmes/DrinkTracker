...
| **Paired-figure table** | `Trends/HealthPairingSection.swift` (`HealthPairingCard`, `HealthPairingOffer`, `HealthPairingCopy`), with `Trends/ComparisonTableParts.swift` | Apple Health beside the log, last on Trends under a `SectionLabel` reading "From Apple Health" (ADR-0050, ADR-0051). One card, one `Grid`, three columns, from the weekday table's own parts and not a second copy of them: the header row is `CardTitle` "Your averages" beside two `columnHead`s — DRINKS and NO DRINKS, primary ink, uppercase, tracked 0.6 — on one baseline, 8pt then a full-strength `Divider`; the metric row is the name in subheadline over "36 and 48 nights" in caption with the numerals in `.rowCount` primary, then the two averages in `.rowFigure` (rounded, tabular, the reader's own) with "bpm" in default-SF caption secondary beside each, 8pt vertical padding; then a hairline and `SourceDisclosure` ("From Apple Health, last 13 weeks", the span following `TrendRange`) over a note that defines the two columns to ADR-0048's buckets. **Both figures, never a delta**: no colour, no bold on one of the pair, no arrow, no sign, no chart, and a range change crossfades the figures (`.contentTransition(.opacity)`, `.smooth(0.22)`) rather than rolling them. The numeric columns size to their content on purpose — at the drawing's two fixed 84pt columns the leading column on a 375pt phone would have 119pt (111 at the weekday table's 88pt scaled metric) and "Resting heart rate" (122.1pt) would wrap; measured with CoreText in the card's own tabular figures, the header row is 213.9pt natural and the leading column keeps 170.9pt on a 375pt phone (160.4 against three-digit figures), one line everywhere down to that width. The card draws figures only under the range they were read for (`HealthPairingModel.Loaded`), so a range change keeps the previous card until the new read lands. From `.xLarge` up it folds by `ComparisonTable.folds` to the title, the name and the two VoiceOver sentences as body text, so nothing at a large size is newly worded (rendered at `.xLarge` and AX5, not reasoned). The metric row is one VoiceOver stop, its label on the name cell, the heads and figure cells hidden; the label speaks the long phrases the heads stand for. The whole section renders nothing — no heading, no card, no stand-in — when no row clears its gate (fourteen nights with a value in each bucket, ADR-0048) or the read returns nothing; the section resolves the row's and the offer's conditions once and the heading is their literal disjunction. Appears with `.smooth(0.25)` opacity, no skeleton and no spinner. The **offer** is the same card with "– –" in tertiary ink for figures, the log's own counts in caption, a hairline, three body sentences, a full-width `SUButton` primary ("Show this on Trends") and a 44pt subtle one ("Not now"); the placeholders say nothing to VoiceOver and the body is the card's label. The only brand colour on the feature's surfaces is the switch's tint and the offer's two buttons (accent fill, accent ink). Design reference: `docs/design/health-pairing/` — `Trends Health Table.dc.html`, `The Offer.dc.html`, `Settings Switches.dc.html`, and the README's spec table; five of its strings changed in the build, listed in ADR-0050 |
