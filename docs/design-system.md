...
type, Liquid Glass surfaces, SF Symbols for the chrome (one exception since
ADR-0040: the Today tab wears the untyped drink's own drop) — and, since ADR-0036, one
...
| **Tab bar** | `Navigation/AppTabs.swift` | The system's iOS 26 tab bar, never a drawn one: five tabs, each a glyph over its name — the app's own drop (`tally.standard`) for Today, then `calendar`, `chart.bar`, `list.bullet`, `gearshape`, drawn by the bar in their fill variant where one exists (`chart.bar`, `gearshape`) in every state — selection is accent ink and a neutral glass capsule, never a change of form. One `NavigationStack` per tab, owned here; opens on Today every launch, nothing stored; no minimise-on-scroll, no badge; inactive tabs in primary ink, the selected glyph and word in the accent, the capsule the system's glass (ADR-0040). Design reference: `docs/design/bottom-nav/` |
