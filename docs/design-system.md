...
| **Comparisons section** | `Trends/ComparisonsSection.swift`, `Trends/PopulationComparisonCards.swift`, `Trends/WeekendComparisonCard.swift`, `Calendar/YearComparisonCard.swift`, `Trends/PopulationReferenceCopy.swift` (`SourceDisclosure`) | A `SectionLabel` heading over one card per published comparison, in the order their Settings switches appear, the first two titled with their switch's own words — Weekly average, Drinking days — and the third by its measure, "Days with a drink", at the owner's ruling (its rows already say Friday to Sunday and Monday to Thursday; the switch keeps "Weekend and weekdays"), its two rows and published rate set at the weekday table's own sizes, subheadline labels and a footnote rate (ADR-0038's and ADR-0032's 2026-09-10 amendments). Body-text sentences on glass, one figure each, no chart, no colour, no delta; a 44pt tappable source line per card opens a caption note that says what the figure is and is not, and names the one body of work behind that card. The section resolves all three gates itself and the heading's condition is the literal disjunction of them, so a heading can never outlive its content. The comparison surfaces share one copy set so they cannot drift (ADR-0018, ADR-0030, ADR-0031, ADR-0032); each comparison is the reader's to show (Settings → Comparisons, ADR-0038), and the volume sentence names the survey column it read (ADR-0039) |
...
| **Weekday insights** | `Trends/WeekdayCard.swift`, `Trends/WeekendComparisonCard.swift`, `Trends/ComparisonTableParts.swift` | Two small tables, one card each since ADR-0032's 2026-09-10 amendment. Seven weekday rows over two right-aligned numeric columns, the unit noun stated once in the column head instead of once per row; then, in its own titled card under the Comparisons heading, the user's Friday-to-Sunday / Monday-to-Thursday split beside the published rate, rows being the source's own definition of the weekend and columns being whose figure it is. The rows stay outside that heading because they are the reader's own log, gated by nothing. **Alignment does the comparing the copy refuses to do** — still no rank, no "most", and no chart of seven bars (ADR-0032). The weekday table's numeric columns are `@ScaledMetric` (88) so a two-word head wraps to two lines; the comparison table's columns size to their own content, which keeps "Monday to Thursday" on one line on every 390pt-and-wider iPhone at the default size — on a 375pt phone it wraps to two lines since the rows took the weekday table's subheadline (measured: 141pt needed, 132 left; ADR-0032's later 2026-09-10 amendment); both live in `ComparisonTableParts` so the two cards share one copy of every measured part rather than two. From xLarge up — the first size above the default — the tables fold back to stacked rows and the three reviewed sentences: rendered, not reasoned, because the scaled columns outgrow the names and the label column hyphenated "Wednes-/day" at xLarge and every weekday at xxLarge (ADR-0032's 2026-09-07 amendment). Each weekday row is one VoiceOver stop, its label on the name cell — a modifier on a `GridRow` lands on every cell. The user's counts are rounded and tabular, the published rates default SF — the numeral carries the "your fact / published fact" distinction that no second hue is allowed to (invariant 10). Design reference: `docs/design/Bar chart hover states design/ds/components/weekday-insights.card.html` — a component sheet, not a screen: two panels in a light/dark split, each drawing one block, titled in the tracked uppercase `.sect`, the second "Days with a drink". Its README calls that one card, so it does not anticipate the split into two; the shipped titles are `CardTitle` — "By weekday", and "Days with a drink" for the comparison card, which is titled by its measure at the owner's ruling with no head over its table (ADR-0032's two 2026-09-10 amendments) |
