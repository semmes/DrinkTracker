...
Four vertical strokes and a slash — the canonical tally of five — white on Tally
Deep (`#104281`). Generated by `scripts/make-app-icon.py`; the geometry there is
the reference. Use the mark alone; never lock it up with text, never recolour it,
never show a partial tally (a tally of three reads as a count, and the icon must
not appear to count anything).
