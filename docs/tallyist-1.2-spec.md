...
Style it with slight emphasis (weight or a subtle secondary background). Do **not** escalate color to red or amber, do not add a warning icon, do not add an exclamation. The number is the signal. Urgency styling turns this into a coach, which breaks constraint 3. *(Amended 2026-09-06 by ADR-0034, and this line brought into agreement 2026-09-07, in the colour clause only: the rolling count's chip now carries the intensity ramp from 6 standard drinks up, the calendar's own bands over the window's own standard drinks, so a shade means an amount on the app's one scale. That is calendar-scale colour, not the urgency styling this line forbids: still never red or amber, no icon, no exclamation, no notification, and the card stays optional and off by default. ADR-0017's amendment holds the argument and the measurements; the reopen path is one line — `paceBand` returning nil restores the neutral capsule.)*
