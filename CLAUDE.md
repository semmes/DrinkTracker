...
- The privacy policy has **three** copies that change together (ADR-0024):
  ...
  **The website's policy is not byte-for-byte the canonical one** (ADR-0024,
  amended 2026-09-24): it differs in exactly the lines `WEB_ONLY` names in
  `mirror_docs.py`, an email address for questions and no pointer to the
  repository, by the owner's choice. `render` refuses a canonical edit that
  changes the text one of them replaces, `policy-copies` runs it on every PR,
  and verify still fails on any other difference. So an edit to "Changes to
...
- **A second secret is now required for CI to run at all**, and unlike the
  ...
  `actions/checkout` in `ci.yml` that needs the submodule passes this PAT as `token:`, which checkout
  uses for the superproject *and* its submodules (hence both repos in the
  grant). Without it every job that passes it (all but `published-docs`, which
  checks out without the submodule) fails at its checkout step, which reads as
...
- **GitHub Actions** verifies every PR: domain tests (macOS-native), the iOS
  ...
  check, the published documents build in its three states, the glyph
  generator and `watch-wiring` (the watch verifier's per-target entitlement
  roles, run with `--ci`, ADR-0055). GitHub sometimes **drops the PR webhook
...
- **tallyist.co is live, support is by email, and the website says two lines of the
  policy its own way (2026-09-24; ADR-0024 amended).** The owner attached
  `tallyist.co` to `semmes/Tallyist`'s Pages site (a branch build from `main`, with
  `CNAME` holding the domain) and dropped the site design into
  `docs/design/marketing-design-handoff/` (untracked). **The domain, and what it
  taught:** `semmes.github.io/Tallyist/<path>` now answers 301 to the same path on
  ...
  **The policy:** the owner chose the design's two web-only lines over the same
  change in the canonical policy for 1.4, so the website reads "This policy is kept
  under version control; every change to it, and its date, is recorded." and
  "Questions about this policy can be sent to tallyist@gmail.com." while this
  repository keeps both lines and the app's copy, which has no Contact section,
  keeps the repository sentence. The mirror applies them (`WEB_ONLY`), refuses an
  edit that moves one, and the daily verify holds the website to exactly those
  lines. **The support page** is the
  design's (an email card, its eleven questions) plus the earlier page's how-to
  answers, with the design's copy corrected where a fact needed it (the contract's
  fixed Android wording, "published US surveys", Health among the places the log
  lives, and four more listed in the copy review). Its answers follow the site's
  `platform_state` through Liquid, defaulting to 1; a new CI job, `published-docs`,
  builds both documents with GitHub Pages' Jekyll in states 1 to 3 and checks them.
  **For 1.4:** `docs/app-store-listing.md` now carries `https://tallyist.co/privacy/`,
  `/support/` and `https://tallyist.co/` (Marketing URL) for the 1.4 version, since
  App Store Connect takes these only with a version, and the 1.4 notes are 3,779
  characters with the shorter link. When 1.4 is live, `platform_state` goes to 2 in
  `semmes/Tallyist`'s `_config.yml`. `PrivacyPolicyView.hostedURL` stays on the old
  address, which redirects; moving it ties every later build's link to the domain
  staying registered, the owner's call. **Verified:** the mirror's render and its two
