...
struct DrinkDetailSheet: View {
  ...
  var body: some View {
    ...
    // Three-quarters, or full. No presentation opens at the medium detent
    // any more; each rule here is a fold, one term to reverse.
    //
    // Three-quarters is the opening height of a new entry — the counter's
    // "Add specific" — and of adoption, the two presentations without the
    // time control. At the medium detent the type question fit above the
    // fold and nothing else did: picking a type added size and strength
    // beneath it, and the strength slider — the control this sheet exists
    // for — landed under the pinned figure, reachable only by a drag the
    // sheet never announced (owner's review, 2026-09-10: the sheet should
    // rise "so users can see the abv strength slider as they're logging a
    // drink"). Measured on an iPhone 17 Pro at the default size: the medium
    // detent was 459pt of the 874pt screen, and with Beer picked it ended at
    // the second row of size pills with STRENGTH wholly beneath it;
    // three-quarters is 601pt — 0.75 of the sheet's full height, not of the
    // screen — and holds DRINK, two rows of SIZE and STRENGTH with its range
    // labels above the pinned figure. A fraction rather than a height, so a
    // smaller phone keeps the same share of its screen; `.large` stays as the
    // second detent, so the drag the indicator advertises still has
    // somewhere to go.
    //
    // Full when the presentation carries the time control: an edit, or
    // History's and the calendar's retroactive add, opens *because* of the
    // "When" section, and at the medium detent that section sat below the
    // fold — a reader correcting their history had to know to drag before
    // the one control they came for appeared (owner's review, 2026-09-09).
    //
    // Full at accessibility sizes: measured at AX5 on an iPhone 17 Pro, the
    // medium detent is ~459pt and the sheet's pinned chrome — a two-line
    ...
    // ends. The large detent holds every path with room to scroll.
    .presentationDetents(
      showsTimeControl || dynamicTypeSize.isAccessibilitySize
        ? [.large]
        : [.fraction(0.75), .large]
    )
